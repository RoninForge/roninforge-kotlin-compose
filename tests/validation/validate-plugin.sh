...
# 6. correct-sample must be free of legacy Android / View-system patterns.
#    Skip comment lines so teaching comments naming bad patterns do not match.
CORRECT_HITS=$(grep -rnE '^[[:space:]]*(import androidx\.compose\.material\.[A-Za-z]|GlobalScope\b|findViewById|setContentView\(R\.layout|LiveData<|MutableLiveData|observeAsState|^[[:space:]]*[a-z][A-Za-z0-9_]*!!)' "$REPO_ROOT/tests/fixtures/correct-sample/" 2>/dev/null | head -10 || true)
if [ -z "$CORRECT_HITS" ]; then
    pass "correct-sample fixture is free of banned patterns"
else
    error "correct-sample contains banned patterns (must demonstrate modern Compose/Kotlin):"
    echo "$CORRECT_HITS"
fi

# 7. anti-pattern fixture should contain several tracked violations.
ANTI_HITS=$(grep -rcE '^[[:space:]]*(GlobalScope\b|findViewById|setContentView\(R\.layout|LiveData<|MutableLiveData|observeAsState|kotlinCompilerExtensionVersion)' "$REPO_ROOT/tests/fixtures/anti-pattern-sample/" 2>/dev/null | awk -F: '{s+=$2} END {print s+0}')
